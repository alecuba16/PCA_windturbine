...
ot<-d[,c("date_time","ot")]
ot_daybefore<-ot
#ot_on<-ot_daybefore$date_time[ot_daybefore$ot==1]-as.difftime(7, units="days")
ot_on<-ot_daybefore$date_time[ot_daybefore$ot==1]

seconds_to_aggregate<-86400
ot_on<-as.POSIXct((as.numeric(ot_on)%/%seconds_to_aggregate)*seconds_to_aggregate,origin='1970-01-01')
ot_daybefore$date_time<-as.POSIXct((as.numeric(ot_daybefore$date_time)%/%seconds_to_aggregate)*seconds_to_aggregate,origin='1970-01-01')

ot_daybefore$ot<-0
ot_daybefore$ot[which(ot_daybefore$date_time %in% ot_on)]<-1
...
dt<-dt[keep]
ot<-ot[keep,]
ot_daybefore<-ot_daybefore[keep,]
d<-d[,apply(d, 2, var, na.rm=TRUE) != 0]
pr<-prcomp(x=d,scale=TRUE,center=TRUE)
#biplot(x = pr)
#ggbiplot(pr,obs.scale = 1, var.scale=1,groups=ot,ellipse=FALSE,circle=FALSE,alpha = 0.1,varname.size = 3)
